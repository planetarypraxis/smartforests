jQuery(function () {
  ...
  const observer = new MutationObserver(function (mutations_list) {
    mutations_list.forEach(() => {
      var upload = jQuery(document).find("section#tab-upload-audio")[0];
      if (upload) {
        // Hide the title, for better UX
        title = jQuery(upload)
          .find('input[name="media-chooser-upload-title"]')
          .closest("li");
        durationInput = jQuery(upload)
          .find('input[name="media-chooser-upload-duration"]')
          .closest("li");
        title.hide();
    ...
    jQuery('input[name="media-chooser-upload-file"]').on(
      "change",
      async function (evt) {
        const fileInput = jQuery(document).find(
          'input[name="media-chooser-upload-file"]'
        );
        ...
        if (!!(data?.duration && !isNaN(data?.duration))) {
          ...
          durationInput.show();
          jQuery('input[name="media-chooser-upload-duration"]')
            .val(duration)
            .attr("disabled", true);
...
function onFail(fileInput) {
  const alertText =
    "Something is wrong with that file, or it's the wrong file type. Supported formats are MP3, OGG or WAV";
  fileInput.val(null);
  alert(alertText);
}
